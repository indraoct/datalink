...
@section('js')
	...
	function renderData()
	{
		$('#custId').val( '{{$customer->id_customer}}' ); 
		$('#custName').val( '{{$customer->customer_name}}' ); 
		$('#custCategory').select2('val', '{{$customer->id_category}}');
		$('#salesPerson').select2('val', '{{$customer->id_sales_person}}');
		$('#desc').val( '{{$customer->description}}' ); 
		$('#addr').val( '{{$customer->address}}' ); 
		$('#phone').val( '{{$customer->telephone}}' ); 
		$('#fax').val( '{{$customer->fax}}' ); 
		
		$('#bank').val( '{{$customer->bank_name}}' ); 
		$('#bankAcct').val( '{{$customer->bank_acct_no}}' ); 
		$('#acctName').val( '{{$customer->bank_acct_name}}' ); 
		$('#npwp').val( '{{$customer->npwp}}' ); 
		$('#pkp').val( '{{$customer->pkp}}' ); 
		// $('#pkpDate').val( '{{$customer->pkp_date}}' ); 
		// $('#tax').prop( 'checked',{{$customer->tax}} ); 
		
		$('#rCcy').select2('val', '{{$customer->currency}}');
		$('#rTop').select2('val', '{{$customer->term_of_payment}}');
		$('#rInterval').val( '{{$customer->credit_interval}}' ); 
		$('#rPlafond').val( '{{ ($customer->credit_plafond) ? displayNumeric($customer->credit_plafond) : ''}}' ); 
		$('#rReceive').select2('val', '{{$customer->account_receivable}}');
...
<!-- BEGIN PAGE HEADER-->
	...
	<!-- BEGIN PAGE CONTENT-->
												...
												<div class="row">
													<div class="col-md-6">
														<div class="form-group">
															...
															<div class="col-md-6">
																{{ Form::hidden('id','',array('id'=>'custId')) }}
																{{ Form::text('custName','',array('id'=>'custName','class'=>'form-control')) }}
															</div>
														</div>
														<div class="form-group">
															<label class="col-md-4 control-label">Customer Category</label>
															<div class="col-md-6">
																{{Form::select('custCategory',$listCategory,0,array('id'=>'custCategory','class'=>'form-control select2')) }}
															</div>
														</div>
													</div>
													<!--/span-->
												...
												<div class="row">
													<div class="col-md-6">
														<div class="form-group">
															...
															<div class="col-md-6">
																{{Form::select('salesPerson',$listSales,0,array('id'=>'salesPerson','class'=>'form-control select2')) }}
															</div>
														</div>
													</div>
													<!--/span-->
												...
												<div class="row">
													...
													<div class="col-md-6">
														...
														<div class="form-group">
															...
															<div class="col-md-6">
																{{Form::select('rReceive',$listAcctReceivable,0,array('id'=>'rReceive','class'=>'form-control select2')) }}
															</div>
														</div>
													</div>
												</div>
				...
				<div id="button" class="modal-footer">
					<a href="{{ URL::to('/') }}/customer"><button type="button" class="btn default" >Back</button></a>
