...
class DeliveryOrderModel {
	...
	public function selectionBOM()
	{
		$data = DB::table($this->_bomTable.' as a')
										->select(array('a.id_bom',DB::raw('CONCAT(\'BOM-\',a.bom_no) as bom_no')) )
										->where('a.is_deleted',0)
										->where('a.status',0)
										->where('a.owner', Crypt::decrypt(Session::get('owner')))
										->orderBy('a.bom_no')
										->get();
		return listArrayEncodedID($data, 'id_bom', 'bom_no');
	}
	...
	public function insertData($data)
	{
		try
		{
			...
			self::updateStatusBom($data['idBom'],1);
			//INSERT TRX
		...
		if(is_array($data))
		{
			foreach($data as $key=>$row)
			{
				$insertArr = array(
					...
					'qty_open'		=> defaultNumeric($row['qty_open']),
	...
	public function updateStatusBom($id,$status)
	{
		$updateArr = array(
			'status'	=> $status,
		);
		DB::table($this->_bomTable)
							->where('id_bom',$id)
							->update($updateArr);
		return true;
	}
