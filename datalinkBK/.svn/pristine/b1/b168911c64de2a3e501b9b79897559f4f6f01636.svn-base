...
class ItemModel {
    ...
    public function updateData($data) {
        ...
        try {
            ...
            $updateItem = array(
                ...
                'tax_type' => $data['tax'],
                'status' => $data['status'],
                'min_stock' => defaultNumeric($data['minimumStock']),
                'max_stock' => defaultNumeric($data['maximumStock']),
                'status' => $data['status'],
                'is_deleted' => 0,
                'created_by' => Session::get('user_name'),
                'created_at' => DB::raw('now()')
    ...
    public function getLike($keyword, $limit) {
        $query = DB::table($this->_table)
                ->select(array($this->_id . ' as id', 'item_code', 'item_name'))
    ...
    public function getRow($id) {
        $query = DB::table($this->_table)
                ->select(array($this->_id, 'item_code', 'item_name', 'cogs_price', $this->_table .'.'. $this->_id_unit, 'unit_name'))
                ->leftJoin($this->_table_unit, $this->_table . '.' . $this->_id_unit, '=', $this->_table_unit . '.' . $this->_id_unit)
                ->where($this->_id, $id);
        return $query->first();
    }
