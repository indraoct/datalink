...
@section('js')
	...
	function construct() {
		...
		var grid = new Datatable();
				...
				grid.init({
					...
					dataTable: { // here you can define a typical datatable settings from http://datatables.net/usage/options 
						"ajax": {
							"url": "{{ URL::to('/') }}/inventory/good_receipt/get_data", // ajax source
						},
						"order": [
							[1, "asc"]
		...
		$("#modal").on('click', "#procces", function(){
			var id = $("#id_po").val();
			if(id)
				window.location.href ="{{ URL::to('/') }}/inventory/good_receipt/new/"+id;
			else
				...
				{{ Form::hidden('report','',array()) }}
					<div class="table-container">
						...
						<tr role="row" class="filter">
							...
							<td>
								{{ Form::select('filter[id_vendor]',$listVendor,null,array('class'=>'form-control select2 form-filter input-sm')) }}
							</td>
							<td>
								<input type="text" class="form-control form-filter input-sm" name="filter[no_ref]">
							</td>
							<td>
						...
						<form role="form" id="form2" class="form-horizontal" action="#">
							<div class="form-body">
								<div class="form-group">
									<label class="col-md-3 control-label">PO No <font color="red"><strong> * </strong></font></label>
									<div class="col-md-5">
										{{ Form::hidden('act','',array('id'=>'act')) }}
										{{ Form::select('id_po',$listPO,'',array('id'=>'id_po','class'=>'form-control')) }}
									</div>
								</div>
							</div>
						</form>
