...
class PurchaseInvoiceModel {
    ...
    protected $_table = 't_purchase_invoice';
    protected $_id = 'id_pi';

    public function getData($filter, $limit = null, $offset = null, $sortBy = null, $sortDir = null) {
        $data['id'] = $this->_id;

        $tableColumn = array($this->_id, 'pi_no','inv_ref_no', 'pi_date', 'due_date', 'id_vendor','currency','total','status');
        ...
        if ($filter) {
            if (isset($filter['pi_no'])){
                $query->where('pi_no', 'LIKE', '%' . $filter['pi_no'] . '%');
            }
            if (isset($filter['inv_ref_no'])){
                $query->where('inv_ref_no', 'LIKE', '%' . $filter['inv_ref_no'] . '%');
            }
            if ($filter['tanggal_dari']){
                $query->where('pi_date', '>=', defaultDate($filter['tanggal_dari']));
            }
            if ($filter['tanggal_hingga']){
                $query->where('pi_date', '<=', defaultDate($filter['tanggal_hingga']));
            }
            if ($filter['tanggal_due_dari']){
                $query->where('due_date', '>=', defaultDate($filter['tanggal_due_dari']));
            }
            if ($filter['tanggal_due_hingga']){
                $query->where('due_date', '<=', defaultDate($filter['tanggal_due_hingga']));
            }
