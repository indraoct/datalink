...
class GoodReceiptModel {
	...
	public function selectionPO()
	{
		$data = DB::table($this->_poTable.' as a')
										->select(array('a.id_po',DB::raw('CONCAT(\'PO-\',a.po_no) as po_no')) )
										->where('a.is_deleted',0)
										->where('a.status',0)
										->where('a.owner', Crypt::decrypt(Session::get('owner')))
										->orderBy('a.po_no')
										->get();
		return listArrayEncodedID($data, 'id_po', 'po_no');
	}
	...
	public function insertData($data)
	{
		try
		{
			...
			$modelPO->updateStatus($data['idPo'],1);
			//INSERT TRX
			 ...
			 $paramStokAkun = array(
				...
				'trx_code'	=> 'GR',
				'trx_date'		=> defaultDate($data['trx_date']),
				'id_ref'			=> $idMaster, // increment
				'no_ref'		=> $data['trx_no'], // 14001o
				'notes' 		=> 'Good Receipt',
				'created_at'	=> DB::raw('now()')
			);
