...
class ProjectBoMController extends BaseController 
{
	protected $_defaultModel = 'BomModel';
	protected $_pageTitle = 'Project';
	protected $_menuId = 'bom';
    ...
    public function index($idProject)
	{
		...
		if (Request::isMethod('post'))
		{
			...
			// if( !empty($insertData['master']) && !empty($insertData['detail']))
			{
				...
				$qtModel = new SalesModel();
				$getLatestSales = $qtModel->getLatestSales($idProject);
		...
		$param = array(
							...
							'title_desc'	=> 'Project Bill of Material',
							'page'			=> 'bom',
							'listCcy'		=> $listCcy,
							'quot'			=> $insertData,
							'data'			=> $data['data'][0],
        ...
        return View::make('modules.project.bom',$param);
    }
